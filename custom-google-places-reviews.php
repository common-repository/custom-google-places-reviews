 ...
 * Description:       Custom Google Places Reviews Display your Google business pages reviews on your website just by adding a shortcode [google_reviews]
 * Version:           1.0.1
...
function custom_cgpr_google_palce_reviews_post_type() {
    ...
    $labels = array(
        'name'                  => _x( 'Google Place Reviews', 'Post Type General Name', 'cgpr_googlepalcereviews' ),
        'singular_name'         => _x( 'Google Place Review', 'Post Type Singular Name', 'cgpr_googlepalcereviews' ),
        'menu_name'             => __( 'Google Place Reviews', 'cgpr_googlepalcereviews' ),
        'name_admin_bar'        => __( 'Google Place Review', 'cgpr_googlepalcereviews' ),
        'archives'              => __( 'Item Archives', 'cgpr_googlepalcereviews' ),
        'attributes'            => __( 'Item Attributes', 'cgpr_googlepalcereviews' ),
        'parent_item_colon'     => __( 'Parent Item:', 'cgpr_googlepalcereviews' ),
        'all_items'             => __( 'All Items', 'cgpr_googlepalcereviews' ),
        'add_new_item'          => __( 'Add New Item', 'cgpr_googlepalcereviews' ),
        'add_new'               => __( 'Add New', 'cgpr_googlepalcereviews' ),
        'new_item'              => __( 'New Item', 'cgpr_googlepalcereviews' ),
        'edit_item'             => __( 'Edit Item', 'cgpr_googlepalcereviews' ),
        'update_item'           => __( 'Update Item', 'cgpr_googlepalcereviews' ),
        'view_item'             => __( 'View Item', 'cgpr_googlepalcereviews' ),
        'view_items'            => __( 'View Items', 'cgpr_googlepalcereviews' ),
        'search_items'          => __( 'Search Item', 'cgpr_googlepalcereviews' ),
        'not_found'             => __( 'Not found', 'cgpr_googlepalcereviews' ),
        'not_found_in_trash'    => __( 'Not found in Trash', 'cgpr_googlepalcereviews' ),
        'featured_image'        => __( 'Featured Image', 'cgpr_googlepalcereviews' ),
        'set_featured_image'    => __( 'Set featured image', 'cgpr_googlepalcereviews' ),
        'remove_featured_image' => __( 'Remove featured image', 'cgpr_googlepalcereviews' ),
        'use_featured_image'    => __( 'Use as featured image', 'cgpr_googlepalcereviews' ),
        'insert_into_item'      => __( 'Insert into item', 'cgpr_googlepalcereviews' ),
        'uploaded_to_this_item' => __( 'Uploaded to this item', 'cgpr_googlepalcereviews' ),
        'items_list'            => __( 'Items list', 'cgpr_googlepalcereviews' ),
        'items_list_navigation' => __( 'Items list navigation', 'cgpr_googlepalcereviews' ),
        'filter_items_list'     => __( 'Filter items list', 'cgpr_googlepalcereviews' ),
    );
    $args = array(
        'label'                 => __( 'Google Place Review', 'cgpr_googlepalcereviews' ),
        'description'           => __( 'Google Place Review', 'cgpr_googlepalcereviews' ),
        'labels'                => $labels,
...
function wpt_add_cgpr_metaboxes() {
    add_meta_box(
        ...
        __( 'Google Place Reviews', 'cgpr_googlepalcereviews' ),
        'wpt_cgpr_google_palce_reviews_location',
...
function wpt_cgpr_google_palce_reviews_location() {
    ...
    $google_palce_id = get_post_meta( $post->ID, 'google_api_key', true );
    $google_api_key = get_post_meta( $post->ID, 'google_palce_id', true );
    // Output the field
    echo '<h2 class="hndle ui-sortable-handle" style="padding-left: 0;"><span>'. __( 'Google API Key', 'cgpr_googlepalcereviews' ) .'</span></h2>';
    echo '<input type="text" name="google_api_key" value="' . esc_textarea( $google_api_key )  . '" class="widefat">';
	echo '<h2 class="hndle ui-sortable-handle" style="padding-left: 0;"><span>'. __( 'Google Place ID', 'cgpr_googlepalcereviews' ) .'</span></h2>';
	...
	echo '<h1 class="hndle ui-sortable-handle" style="padding-left: 0;">'. __( 'Get a Google API Key', 'cgpr_googlepalcereviews' ) .'</h1><p style="font-size:16px;">1. Go to the <a href="https://console.developers.google.com/flows/enableapi?apiid=places_backend,maps_backend,geocoding_backend,directions_backend,distance_matrix_backend,elevation_backend&keyType=CLIENT_SIDE&reusekey=true&pli=1" target="_blank">'. __( 'Get a Google API Key', 'cgpr_googlepalcereviews' ) .'</a></p><p style="font-size:16px;">2. Create or select a project.</p><p style="font-size:16px;">3. Click Continue to enable the API and any related services..</p><p style="font-size:16px;">4. On the Credentials page, Select API Key get a Browser key (and set the API Credentials) </p>';
...
function set_custom_edit_cgpr_google_palce_reviews_columns($columns) {
    ...
    $columns['cgpr_google_palce_reviews_shortcode'] = __( 'Shortcode', 'cgpr_googlepalcereviews' );
    $columns['google_palce_id'] = __( 'Google Place ID', 'cgpr_googlepalcereviews' );
    $columns['google_api_key'] = __( 'Google API Key', 'cgpr_googlepalcereviews' );
	$new = array();
